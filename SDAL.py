        ...
        for img, loss in failure_cases:
            ...
            try:
                ...
                labels, example_images = oracle(
                    ...
                    num_containers=hyp.get('num_containers', 5),
                    blender_bin=hyp.get('blender_bin', '/snap/bin/blender'),
                    oracle_runs_root=str((Path(log_dir) / f"cycle_{i+1}_oracle_runs").resolve()),
                    keep_oracle_artifacts=bool(hyp.get('keep_oracle_artifacts', False)),
                    failure_case_id=Path(informative_image_path).stem,
